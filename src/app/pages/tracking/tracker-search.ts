...
@Component({
    ...
    standalone: true,
    imports: [CommonModule, FormsModule, RouterModule, ButtonModule, InputTextModule, TopbarWidget, FooterWidget, ToastModule],
                    ...
                    <div class="w-full max-w-lg">
                        <div class="surface-card p-8 rounded-2xl bg-surface-0 dark:bg-surface-900 shadow-xl border border-surface-200 dark:border-surface-800 text-center">
                            <div class="mb-8">
                                ...
                                <h1 class="text-3xl font-bold text-surface-900 dark:text-surface-0 mb-3">Track Your Order</h1>
                                <p class="text-surface-600 dark:text-surface-300">Enter your Quote Token provided in your confirmation email to check the status.</p>
                            </div>

                            <div class="flex flex-col gap-4">
                                <div class="flex flex-col gap-2 text-left">
                                    <label for="token" class="font-medium text-surface-900 dark:text-surface-0 ml-1">Quote Token / Order ID</label>
                            ...
                            <div class="mt-8 pt-6 border-t border-surface-100 dark:border-surface-800">
                                <p class="text-sm text-surface-500">Need assistance? <a routerLink="/" fragment="contact" class="text-primary hover:underline cursor-pointer">Contact Support</a></p>
                            </div>
                        </div>
                    </div>
                </div>
    ...
    trackOrder() {
        if (!this.token || !this.token.trim()) {
            this.messageService.add({
                ...
                detail: 'Please enter your quote token'
            });
            return;
        }
