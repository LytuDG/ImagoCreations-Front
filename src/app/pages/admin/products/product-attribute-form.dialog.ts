        ...
        <div class="flex flex-col gap-5">
            ...
            @if (existingAttributes.length) {
                ...
                <label class="block font-semibold mb-3 text-gray-700">
                    ...
                    ({{ existingAttributes.length }} attribute{{ existingAttributes.length !== 1 ? 's' : '' }})
                    </span>
                </label>
            ...
            <div class="mb-6">
                ...
                @if (existingAttributes.length) {
                    <span class="ml-2 text-sm font-normal text-gray-500">
                    (Attributes already assigned are hidden)
                    </span>
                }
                </label>
                <p-multiSelect
  ...
  loadAvailableAttributes(): void {
    ...
    this.attributeService.getAttribute({}).subscribe({
      next: (response) => {
        ...
        const allAttributeIds = this.availableAttributes
          .map(attr => attr.id!)
          .filter(id => id && !this.attributeValuesMap[id]) as string[];
