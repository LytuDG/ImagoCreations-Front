...
@Component({
    ...
    standalone: true,
    imports: [CommonModule, FormsModule, ButtonModule, InputTextModule, SelectModule, ToggleButtonModule, DialogModule],
    template: `
        <p-dialog [(visible)]="visible" [style]="{ width: '500px' }" [header]="isEditMode ? 'Editar Atributo' : 'Nuevo Atributo'" [modal]="true" [draggable]="false" [closable]="!saving" (onHide)="onHide()">
            <ng-template #content>
                <div class="flex flex-col gap-4 p-fluid">
                    <!-- Nombre del Atributo -->
                    <div class="field">
                        <label for="name" class="block font-bold mb-2">Nombre *</label>
                        <input type="text" pInputText id="name" [(ngModel)]="localAttribute.name" required class="w-full" placeholder="Ej: Color, Talla, Material" [disabled]="saving" />
                    </div>

                    <!-- Tipo de Input -->
                    <div class="field">
                        <label for="inputType" class="block font-bold mb-2">Tipo de Campo *</label>
                        <p-select [options]="inputTypeOptions" optionLabel="label" optionValue="value" [(ngModel)]="localAttribute.inputType" placeholder="Selecciona un tipo" class="w-full" [disabled]="saving" (onChange)="onInputTypeChange()">
                        ...
                        @if (selectedTypeConfig) {
                            <small class="text-color-secondary block mt-2">
                                ...
                                @if (selectedTypeConfig.supportsValues) {
                                    <span class="ml-2 text-primary font-medium"> (Requiere valores predefinidos) </span>
                                }
                            </small>
                        }
                    </div>
                    ...
                    <div class="justify-start">
                        ...
                        <div [style]="{ 'grid-template-columns': 'repeat(2, minmax(0, 0.3fr))' }" class="grid gap-4 p-3 surface-border border-round">
                            ...
                            <div class="flex justify-start">
                                <p-toggleButton [(ngModel)]="localAttribute.required" onLabel="Sí" offLabel="No" onIcon="pi pi-check" offIcon="pi pi-times" [style]="{ width: '80px' }" [disabled]="saving" />
                            </div>

                            <!-- Fila 2 -->
                            <span class="font-medium flex items-center">Reutilizable</span>
                            <div class="flex justify-start">
                                <p-toggleButton [(ngModel)]="localAttribute.reusable" onLabel="Sí" offLabel="No" onIcon="pi pi-check" offIcon="pi pi-times" [style]="{ width: '80px' }" [disabled]="saving" />
                            </div>

                            <!-- Fila 3 -->
                            <span class="font-medium flex items-center">Visible B2B</span>
                            <div class="flex justify-start">
                                <p-toggleButton [(ngModel)]="localAttribute.visibleB2B" onLabel="Sí" offLabel="No" onIcon="pi pi-check" offIcon="pi pi-times" [style]="{ width: '80px' }" [disabled]="saving" />
                            </div>

                            <!-- Fila 4 -->
                            <span class="font-medium flex items-center">Visible B2C</span>
                            <div class="flex justify-start">
                                <p-toggleButton [(ngModel)]="localAttribute.visibleB2C" onLabel="Sí" offLabel="No" onIcon="pi pi-check" offIcon="pi pi-times" [style]="{ width: '80px' }" [disabled]="saving" />
                            </div>
                        </div>
                    </div>
                    ...
                    @if (showValuesSection && attribute?.id) {
                        <div class="field mt-4">
                            <div class="flex items-center justify-content-between mb-3">
                                ...
                                <p-button label="Gestionar Valores" icon="pi pi-cog" size="small" (onClick)="manageAttributeValues()" [disabled]="saving" />
                            </div>
                            <div class="p-3 border surface-border border-round bg-surface-50">
                                <div class="text-center py-3">
                                    ...
                                    <span class="text-color-secondary"> Este atributo requiere valores predefinidos. Haz clic en "Gestionar Valores" para agregarlos. </span>
                                </div>
                            </div>
                        </div>
                    }
            ...
            <ng-template #footer>
                <div class="flex justify-content-end gap-2">
                    <p-button label="Cancelar" icon="pi pi-times" text (click)="onCancel()" [disabled]="saving" />
                    <p-button label="Guardar" icon="pi pi-check" (click)="onSave()" [disabled]="!localAttribute.name || !localAttribute.inputType || saving" [loading]="saving" />
                </div>
            </ng-template>
        </p-dialog>
    `
